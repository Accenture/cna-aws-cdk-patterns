...
export class StaticWebsite extends Construct {
  ...
  constructor(scope: Construct, id: string, props: StaticWebsiteProps) {
    ...
    this.contentBucket = this.createEncryptedBucket(
      "content-bucket",
      `site-${props.name}-content`
    );

    // Create the S3 bucket for storing the logs from CloudFront
    this.loggingBucket = this.createEncryptedBucket(
      "logging-bucket",
      `site-${props.name}-logs`
    );
  ...
  private createCloudFrontDistribution(
    props: StaticWebsiteDistributionProps
  ): Distribution {
  ...
  private createCloudFrontDistributionWithCustomDomain(
